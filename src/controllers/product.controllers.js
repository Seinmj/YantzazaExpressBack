...
const getProductByIdWeb = async (req, res) => {
    const { enterprise_id, idProducto } = req.params;
    try {
        const query = `
        SELECT 
            pr.product_id,
        ...
        WHERE 
            e.enterprise_id = $1 AND pr.product_id = $2
        `;

        const values = [enterprise_id, idProducto];
        const { rows } = await pool.query(query, values);
